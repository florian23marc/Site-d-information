...
class CartController extends AbstractController
{
    ...
    {
        // Récupération du panier à supprimer
        $cart = $this->em->getRepository(Cart::class)->find($id);

        // Vérifier si le panier existe et appartient à l'utilisateur actuel
        if (!$cart || $cart->getUser() !== $this->getUser()) {
            // Gérer l'erreur ou rediriger si le panier n'existe pas ou n'appartient pas à l'utilisateur
            //throw $this->createNotFoundException('Panier non trouvé ou accès non autorisé.');
            return $this->redirectToRoute('cart');
        }

        // Supprimer d'abord tous les éléments (CartItems) du panier
        foreach ($cart->getCartItems() as $cartItem) {
            $this->em->remove($cartItem);

        }

        // Supprimer ensuite le panier lui-même
        $this->em->remove($cart);
        $this->em->flush();

        // Rediriger vers la liste des paniers ou une autre page appropriée
        return $this->redirectToRoute('cart');
    }
